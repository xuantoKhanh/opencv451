...
public:
    ...
    */
    ...
    @note Result or stored exception can be fetched only once.
    */
    bool get(OutputArray dst, int64_t timeoutNs) const;

    CV_WRAP inline
    bool get(OutputArray dst, double timeoutNs) const { return get(dst, (int64_t)timeoutNs); }

    bool wait_for(int64_t timeoutNs) const;

    CV_WRAP inline
    bool wait_for(double timeoutNs) const { return wait_for((int64_t)timeoutNs); }
    ...
    {
        return get(dst, (int64_t)(std::chrono::nanoseconds(timeout).count()));
    }

    template<typename _Rep, typename _Period>
    inline bool wait_for(const std::chrono::duration<_Rep, _Period>& timeout)
    {
        return wait_for((int64_t)(std::chrono::nanoseconds(timeout).count()));
    }
